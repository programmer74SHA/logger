...
type scannerService struct {
	...
	assetRepo    assetPort.Repo // Add asset repo for firewall asset creation
	db           *gorm.DB
}
...
func NewScannerService(repo scannerPort.Repo, db *gorm.DB) scannerPort.Service {
	return &scannerService{
		...
		assetRepo:    storage.NewAssetRepo(db), // Initialize asset repo
		db:           db,
	}
}
...
func (s *scannerService) validateSwitchScanner(scanner *domain.ScannerDomain) error {
	...
	if scanner.IP == "" {
		return fmt.Errorf("switch scanner requires IP address")
	}
	if scanner.Username == "" {
		return fmt.Errorf("switch scanner requires username")
	}
	if scanner.Password == "" {
		return fmt.Errorf("switch scanner requires password")
	}

	// Set default protocol if not provided
	if scanner.Protocol == "" {
		scanner.Protocol = "SSH"
	}

	// Validate protocol (SSH only for now)
	if strings.ToUpper(scanner.Protocol) != "SSH" {
		return fmt.Errorf("invalid protocol: %s. Only SSH is supported", scanner.Protocol)
	}
	...
	if portNum, err := strconv.Atoi(scanner.Port); err != nil || portNum < 1 || portNum > 65535 {
		return fmt.Errorf("invalid port number: %s. Must be between 1 and 65535", scanner.Port)
	}

	// Validate device type
	if scanner.Type == "" {
		return fmt.Errorf("device type is required (e.g., 'Cisco', 'Juniper', 'Huawei')")
	}
	...
	for _, validType := range validDeviceTypes {
		if strings.EqualFold(scanner.Type, validType) {
			// Normalize the type
			scanner.Type = validType
			isValidType = true
			break
		}
	}
	if !isValidType {
		return fmt.Errorf("invalid device type: %s. Valid types are: %v", scanner.Type, validDeviceTypes)
	}
