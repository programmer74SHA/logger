...
func applyAssetFiltersToQuery(query *gorm.DB, filters *domain.AssetFilters) *gorm.DB {
	...
	if filters.Type != "" {
		query = query.Where("asset_type = ?", filters.Type) // Changed from "type" to "asset_type"
		logger.Debug("Repository: Applied type filter: %s", filters.Type)
	}
	if filters.IP != "" {
		query = query.Joins("JOIN ips ON assets.id = ips.asset_id AND ips.deleted_at IS NULL").
			Where("ips.ip_address LIKE ?", "%"+filters.IP+"%").
			Group("assets.id")
...
// CreateWithVendor creates an asset with proper vendor handling
func (r *assetRepository) CreateWithVendor(ctx context.Context, asset domain.AssetDomain, scannerType string) (domain.AssetUUID, error) {
	// Set the discovered_by field if scanner type is provided
	if scannerType != "" {
		asset.DiscoveredBy = scannerType
	}

	logger.InfoContextWithFields(ctx, "Repository: Creating asset with vendor handling", map[string]interface{}{
		"asset_id":      asset.ID.String(),
		"asset_name":    asset.Name,
		"hostname":      asset.Hostname,
		"ip_count":      len(asset.AssetIPs),
	...
	// Convert asset domain to storage model with proper vendor handling
	assetRecord, assetIPs, err := mapper.AssetDomain2StorageWithVendor(asset, r.db, scannerType)
	if err != nil {
		logger.ErrorContext(ctx, "Repository: Failed to convert asset domain to storage with vendor: %v", err)
		return domain.AssetUUID{}, err
	}
...
// InitializeDefaultVendors ensures default vendors exist in the database
func (r *assetRepository) InitializeDefaultVendors(ctx context.Context) error {
	logger.InfoContext(ctx, "Repository: Initializing default vendors")

	defaultVendors := []types.Vendors{
		{VendorName: "Cisco", VendorCode: "CSC"},
		{VendorName: "Fortinet", VendorCode: "FTN"},
	...
	for _, vendor := range defaultVendors {
		...
		err := r.db.WithContext(ctx).Where("vendor_name = ?", vendor.VendorName).First(&existingVendor).Error
		
		if err == gorm.ErrRecordNotFound {
			// Vendor doesn't exist, create it
			if err := r.db.WithContext(ctx).Create(&vendor).Error; err != nil {
				logger.ErrorContext(ctx, "Repository: Failed to create vendor %s: %v", vendor.VendorName, err)
				return err
			}
			logger.InfoContext(ctx, "Repository: Created vendor: %s", vendor.VendorName)
		} else if err != nil {
			logger.ErrorContext(ctx, "Repository: Error checking vendor %s: %v", vendor.VendorName, err)
			return err
		} else {
			logger.DebugContext(ctx, "Repository: Vendor %s already exists", vendor.VendorName)
		}
	}

	logger.InfoContext(ctx, "Repository: Successfully initialized default vendors")
	return nil
}