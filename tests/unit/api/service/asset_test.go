	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(mockService *serviceMocks.MockAssetService) {
				expectedID := uuid.New()
				mockService.On("CreateAsset", mock.Anything, mock.AnythingOfType("domain.AssetDomain"), mock.Anything).
...
func TestAssetService_CreateAsset_ErrorHandling(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockInternalService := new(serviceMocks.MockAssetService)
			mockInternalService.On("CreateAsset", mock.Anything, mock.AnythingOfType("domain.AssetDomain"), mock.Anything).
